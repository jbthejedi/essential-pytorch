...
### `nn.Conv2d` (and why it still matters)
...
`expand` doesn’t copy data; it gives you a read-only ‘broadcasted view’. That’s perfect for things like class tokens and positional embeddings where you reuse the same parameters across the batch.
